...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for p in newPosDist:
                d[p] += newPosDist[p] * self.beliefs[oldPos]
        self.beliefs = d
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        pacmanPosition = gameState.getPacmanPosition()
        jailPostition = self.getJailPosition()
        distribution = DiscreteDistribution()
        for i in self.particles:
            distribution[i] += self.getObservationProb(observation, pacmanPosition, i, jailPostition)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in distribution:
            for i in range(self.numGhosts):
                jailPostition = self.getJailPosition(i)
                distribution[particle] *= self.getObservationProb(observation[i], pacmanPosition, particle[i], jailPostition)
        if distribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            temp = []
            for i in range(self.numParticles):
                temp.append(distribution.sample())
            self.particles = temp

...
def depthFirstSearch(problem):
    ...
    while True:
        ...
        if problem.isGoalState(node[2]):
            return node[0]
        if node[2] not in closed:
            closed.add(node[2])
            for child_node in problem.getSuccessors(node[2]):
                fringe.push((node[0] + [child_node[0]], node[1] + child_node[1], child_node[2]))
...
def uniformCostSearch(problem):
    ...
    while True:
        ...
        if node[2] not in closed:
            closed.add(node[2])
            for child_node in problem.getSuccessors(node[2]):
                fringe.update((node[0] + [child_node[0]], node[1] + child_node[1], child_node[2]),
                              node[1] + child_node[1])
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while True:
        ...
        if node[2] not in closed:
            ...
            for child_node in problem.getSuccessors(node[2]):
                fringe.update((node[0] + [child_node[0]], node[1] + child_node[1], child_node[2]),
                              node[1] + child_node[1] + heuristic(child_node[2], problem))
